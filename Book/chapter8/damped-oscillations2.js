...
function init() {
	...
	ball.velo2D = new Vector2D(0,200);	
	ball.draw(context);
	// create an attractor		
	...
	B = (ball.velo2D.add(A.multiply(gamma*omega0))).multiply(1/omegad);		
	// set up graph
	setupGraph();
	// make the ball move
	t0 = new Date().getTime(); 
...
function plotGraph(){
	...
	var r = A.multiply(Math.cos(omegad*t)).add(B.multiply(Math.sin(omegad*t)));			
	r.scaleBy(Math.exp(-gamma*omega0*t));			
